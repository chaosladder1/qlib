...
def get_date_set(startdate: pd.Timestamp, enddate: pd.Timestamp, interval='1D') -> list:
    """
    ...
    if type(startdate) is str:
         ...
         timeperiod = dt.time(enddate, '%Y-%m-%d').date()- dt.strptime(startdate, '%Y-%m-%d').date()
         enddate= dt.strptime(enddate, '%Y-%m-%d').date()
    ...
    if timeinterval > 100:
        ...
        timeinterval = int(timeinterval/100)
        dates.scale = 100
 # need to make this more sensitive.
        for i in range(timeinterval):

          if i == 0:
            dates.append(enddate)
            new_date = enddate - timedelta(days=100)
        ...
        try:
            ...
            if dates.scale == 100:
                ...
                for i in range(len(dates)):
                  candle = client.public.get_candles(
                      market=symbol,
                      resolution='1DAY',
                      to_iso = dates[i].date()
                    ).data

                  candle = pd.DataFrame(candle["candles"])
                  candles.append(candle)

                  _resp = pd.concat(candles,axis=0 )
...
class CryptoCollector1d(CryptoCollector, ABC):
    def get_instrument_list(self):
        logger.info("get dydx crypto symbols......")
